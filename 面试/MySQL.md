...
-   Consistency（一致性）：在事务开始之前和事务结束以后，数据不会被破坏。
...
利用了MVCC，读取本事务开始时的行数据版本。

4. SERIALIZABLE(可串行化)

可以防止脏读、不可重复读以及幻读。串行化的实现采用的是读写都加锁的原理。
...
**快照读** 由 MVCC 机制来保证不出现幻读，快照读就是单纯的 `SELECT` 语句，但不包括下面这两类 `SELECT` 语句：
